 ...
 *
 * @param {string} text The text to parse an HTML entity
 *
 ...
 * @return {JSX.Element} The editor interface.
 */
export default function Edit({ attributes: attr, setAttributes }) {
	...
	//add a state to track with chapters are open or closed.
	...
	useEffect(() => {
		apiFetch({ path: '/wp/v2/categories?parent=0&per_page=100' })
			...
			.catch((error) =>
				createErrorNotice(
					`Error fetching top-level categories: ${error.message}`,
					{
						type: 'snackbar',
					}
				)
	...
	useEffect(() => {
		...
		Promise.all([
			apiFetch({
				path: `/pau-toc/v1/get-toc?root_id=${parentId}`,
			}),
			apiFetch({
				path: `/wp/v2/categories?parent=${parentId}&per_page=100`,
			}),
		])
			.then(([savedData, freshCategories]) => {
				...
				if (!hasInitialized.current) {
					...
					if (savedData.exists) {
						const existingOrder = savedData.chapterOrder.filter(
							(id) => freshIds.includes(id)
						);
						const newIds = freshIds.filter(
			...
			.catch((error) => {
				createErrorNotice(
					`There was a problem fetching child categories: ${error.message}`,
					{
						type: 'snackbar',
					}
				);
	...
	}, [category, createErrorNotice, setAttributes]);
		...
		Promise.all(fetchPromises)
			.then((results) => {
				...
				results.forEach(({ chapterId, posts }) => {
					...
					const filteredOrder = existingOrder.filter((id) =>
						fetchedPostIds.includes(id)
					);

					// 2. Add new posts (found by API but not in filtered order) to the end
					const missingPosts = fetchedPostIds.filter(
			...
			.catch((error) => {
				createErrorNotice(
					`Error fetching posts for chapters: ${error.message}`,
					{
						type: 'snackbar',
					}
				);
	...
	}, [childCategories, setAttributes, createErrorNotice]);
	...
	const handleMovePost = (chapterIdStr, postId, direction) => {
		...
		if (newIndex >= 0 && newIndex < currentPostOrder.length) {
			...
			// Swap elements
			[newChapterOrder[currentIndex], newChapterOrder[newIndex]] = [
				newChapterOrder[newIndex],
				newChapterOrder[currentIndex],
			];
	...
	const getPostTitle = (chapterIdStr, postId) => {
		...
		return post
			...
			: `[Post ID ${postId} - Title Not Found]`;
	};
	...
	const getChapterName = (chapterId) => {
		...
		return childChapter
			? decodeHTMLEntities(childChapter.name)
			: `[Category ID ${chapterId} - Name Not Found]`;
	};

	const blockProps = useBlockProps({
		className:
			'wp-block-toc-editor p-4 border border-gray-200 rounded-lg bg-gray-50',
	});
	...
	return (
		<div {...blockProps}>
			...
			<InspectorControls>
				<PanelBody
					title={__('Settings', 'pau-table-of-contents-block')}
				>
					<SelectControl
						label={__(
							'Root Category',
			...
			>
					...
					<div className="wp-block-pau-table-of-contents-block-list">
						<ul>
							...
							{orderedChapters.map((chapter, chapterIndex) => {
								...
								const isChapterOpen =
									openChapters[chapterIdStr] || false;
				...
				{!isLoading && category && orderedChapters.length === 0 && (
					<p className="p-3 bg-yellow-100 text-yellow-800 rounded">
						No chapters (child categories) found for this root
						category.
					</p>
				)}
			</div>
