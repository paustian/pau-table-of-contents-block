...
foreach ( $child_categories as $child_cat ) {
	$categories_map[ $child_cat->term_id ] = $child_cat;
}
...
if ( ! empty( $chapter_order ) ) {
	foreach ( $chapter_order as $chap_cat_id ) {
		$chap_cat_id = (int) $chap_cat_id;
		if ( isset( $categories_map[ $chap_cat_id ] ) ) {
			$ordered_chapters[] = $categories_map[ $chap_cat_id ];
			// Remove from map to ensure only missing categories are appended later.
			unset( $categories_map[ $chap_cat_id ] );
		}
	}
	// Append any chapters that were fetched but were not in the saved chapter Order.
...
echo '<div ' . esc_attr( $wrapper_attributes ) . '>';
echo '<ol class="toc-chapters-list">';

foreach ( $ordered_chapters as $chapter ) {
	$chapter_id_str = (string) $chapter->term_id;

	// NEW: Use <details> tag for native accordion functionality.
	// Note: You can add 'open' attribute here if you want chapters to be expanded by default.
	echo '<li class="toc-chapter">';
	echo '<details class="toc-chapter-details">';

	// NEW: The <summary> tag acts as the clickable title/trigger.
	echo '<summary class="toc-chapter-summary">';
	echo '<span class="toc-chapter-name">' . esc_html( $chapter->name ) . '</span>';
	echo '</summary>';
	...
	if ( $chapter_posts->have_posts() ) {
		...
		while ( $chapter_posts->have_posts() ) {
			...
			echo '<a href="' . esc_url( get_permalink() ) . '">' . esc_attr( get_the_title() ) . '</a>';
			echo '</li>';
		}
		echo '</ul>';
		...
		// Display message if no articles found when the chapter is expanded.
		echo '<p class="toc-no-articles">' . esc_html__( 'No articles found in this chapter.', 'pau-table-of-contents-block' ) . '</p>';
	}

	echo '</details>'; // End of chapter accordion.
	echo '</li>'; // End of toc-chapter list item.
}
